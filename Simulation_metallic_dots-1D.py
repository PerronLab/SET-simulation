...
Vd = 0.00005		# Drain voltage

Vp = np.zeros(npts);		# Declare an empty 2D array for the plunger voltage that will be swept
I = np.zeros(npts);            	# Declare an empty 2D array for the current
...
for i in np.arange(0,npts):
	Vp[i] = Vstart + i*delV		# set gate voltage 
	muL = -Vd
...
plt.plot(Vp,I*1e12,'-k', linewidth = 5)
#plt.xlim(Vstart,Vend)
plt.xlim(-0.02,0.1)

plt.ylabel('$I$ (pA)',fontsize=70)
